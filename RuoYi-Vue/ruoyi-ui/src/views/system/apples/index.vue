    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:apples:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:apples:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="applesList" @selection-change="handleSelectionChange">
...
<script>
import { listApples, getApples, delApples, addApples, updateApples } from "@/api/system/apples";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listApples(this.queryParams).then(response => {
        this.applesList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getApples(productType).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.productType != null) {
            updateApples(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addApples(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除VIEW编号为"' + productTypes + '"的数据项？').then(function() {
        return delApples(productTypes);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/apples/export', {
        ...this.queryParams
      }, `apples_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
